...
@Component({
  ...
  styleUrl: './professional-tax-list.component.css'
})
export class ProfessionalTaxListComponent {
  ...
  displayedColumns: string[] = ['id', 'ptfrom', 'ptto', 'ptvalues', 'actions'];

  professionalTaxDataSource = new MatTableDataSource<any>([]);

  excelData: any[]
  constructor(private apiCall: ApiService, private fb: FormBuilder, private router: Router, private route: ActivatedRoute, private toastr: ToastrService, private formBuilder: FormBuilder, private snackbar: MatSnackBar, public dialog: MatDialog) { }
  ...
  saveProfessionalTax() {
    ...
    this.apiCall.apiPostCall(payload, 'saveProfessionalTax').subscribe(data => {
      if (data.status) {
        ...
        // });
        this.toastr.success('PT Created Successfully!');
  ...
  goToTaxCrud(id: any) {
    console.log(id)
    this.router.navigate(['/payroll/professionalEdit', id]);
  }

  deleteData(id) {
    const payload = { id }
    // this.apiCall.apiDeleteCall(payload, 'deleteProfessionalTaxById').subscribe(data => {
    //   this.professionalTaxDataSource.data = data.data;
    //   if (data.result == true) {
  ...
  downloadExcel(): void {
    ...
    FileSaver.saveAs(data, 'ProfessioanlTax.xlsx');
  }
}
